...
- Motion capture: on-device processing vs. server-side, real-time feedback vs. post-session review? (Still open — relevant to Stage 5 technical architecture now that scale target is known to be "small now, thousands later.")
...
- Document/video/photo storage: size limits, retention policy, and storage backend (relevant to Stage 5 — pick storage that can scale to thousands of users without a rewrite).
